...
ERROR_CONTEXT_NOT_INITIALISED = (
    "context has not been initialised properly.  Check environment.py"
)
    ...
    match (op):
        case Comparison.Is:
            assert type(actual) is type(
                expected
            ), f"types don't match {type(actual)} vs {type(expected)}"
...
def read_context(context, collection: str, name: str, default_value: Any = None):
    return (
        ...
        else getattr(context, collection).get(name, default_value)
    )
...
def set_value(context, value: str, name: str):
    assert hasattr(context, "values") and isinstance(
        context.values, dict
    ), ERROR_CONTEXT_NOT_INITIALISED
...
def evaluate(context, expr: str) -> Any:
    ...
    replaced.
    """
    assert hasattr(context, "functions") and isinstance(
        context.functions, dict
    ), ERROR_CONTEXT_NOT_INITIALISED
...
def set_result(context, value: Any, name: str):
    assert hasattr(context, "results") and isinstance(
